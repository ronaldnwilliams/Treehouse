...
public class CheckoutController {
	...
	String checkoutCoupon(Model model, RedirectAttributes redirectAttributes) {
    	...
    	if (purchase != null) {
    		...
    		if (couponCode == null || couponCode.getCode() == "00000") {
			...
			redirectAttributes.addFlashAttribute("flash", new FlashMessage("Sorry! Not sure what happened. Please try again or contact us.", FlashMessage.Status.FAILURE));
	...
	String postCouponCode(Model model, @ModelAttribute(value="couponCode") @Valid CouponCode couponCode, BindingResult result, RedirectAttributes redirectAttributes) {
		if (result.hasErrors() && couponCode.getCode() != "") {
			redirectAttributes.addFlashAttribute("flash", new FlashMessage("Coupon code length must be 5 - 10 characters.", FlashMessage.Status.FAILURE));
			return String.format("redirect:coupon");
		}
		if (couponCode.getCode() == "") couponCode.setCode("00000");
	...
	String checkoutShipping(Model model, RedirectAttributes redirectAttributes) {
    	Purchase purchase = sCart.getPurchase();
    	BigDecimal subTotal = new BigDecimal(0);
    	CouponCode couponCode = sCart.getCouponCode();
	...
	String checkoutBilling(Model model, RedirectAttributes redirectAttributes) {
    	Purchase purchase = sCart.getPurchase();
    	BigDecimal subTotal = new BigDecimal(0);
	...
	String checkoutConfirmation(Model model, RedirectAttributes redirectAttributes) {
    	Purchase purchase = sCart.getPurchase();
    	BigDecimal subTotal = new BigDecimal(0);
		BigDecimal shippingCost = new BigDecimal(0);
		CouponCode couponCode = sCart.getCouponCode();
    	...
    	if (purchase != null) {
			subTotal = computeSubtotal(purchase, couponCode);
			logger.debug(subTotal.toString());
			shippingCost = computeShippingCost(purchase);
			logger.debug(shippingCost.toString());

			BigDecimal orderTotal = subTotal.add(shippingCost);
			logger.debug(orderTotal.toString());
    		...
    		if ( purchase.getCreditCardNumber() != null ) {
				model.addAttribute("hiddenCreditCard", "XXXXXXXXXX" + purchase.getCreditCardNumber().substring(purchase.getCreditCardNumber().length()-4));
			}
    	} else {
    		logger.error("No purchases Found!");
			redirectAttributes.addFlashAttribute("flash", new FlashMessage("Sorry! Not sure what happened. Please try again or contact us.", FlashMessage.Status.FAILURE));
	...
	private BigDecimal computeSubtotal(Purchase purchase, CouponCode couponCode) {
		...
		if (couponCode.getCode() != null && !couponCode.getCode().isEmpty() && couponCode.getCode() != "00000") {
