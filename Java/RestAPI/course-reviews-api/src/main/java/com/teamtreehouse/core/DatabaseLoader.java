...
public class DatabaseLoader implements ApplicationRunner {
    ...
    public void run(ApplicationArguments args) throws Exception {
        ...
        course.addReview(new Review(5, "You ARE awesome!!!"));
        courses.save(course);
        String[] templates = {
        ...
        users.save(students);
        users.save(new User("rnw1990","Ronald", "Williams",  "password", new String[] {"ROLE_USER", "ROLE_ADMIN"}));
        List<Course> simulatedCourses = new ArrayList<>();
        IntStream.range(0, 100)
                .forEach(i -> {
                    String template = templates[i % templates.length];
                    String technology = technologies[i % technologies.length];
                    String title = String.format(template, technology);
                    Course c = new Course(title, "http://example.com");
                    Review review = new Review((i % 5) + 1, String.format("More %s please!!!", technology));
                    review.setReviewer(students.get(i % students.size()));
                    c.addReview(review);
                    simulatedCourses.add(c);
                });
        courses.save(simulatedCourses);
